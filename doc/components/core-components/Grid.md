...
`colSpan` | A number specifying how many columns the cell should span (only applicable if the columns have been defined in the `subRows` property).
`rowSpan` | A number specifying how many rows the cell should span (only applicable if the columns have been defined in the `subRows` property).
