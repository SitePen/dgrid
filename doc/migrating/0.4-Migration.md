...
Observing a store for changes is still very easy thanks to the [`dstore/Trackable`](https://github.com/SitePen/dstore/blob/master/docs/Collection.md#track) API. Simply call `track` and then register listeners:

```js
var TrackableMemory = declare([Memory, Trackable]);
var store = new TrackableMemory({data: ...});
var tracked = store.track();
...
Or take an existing store and create a trackable store from that:

```js
var tracked = Trackable.create(existingStore).track();
```
