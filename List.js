...
function(arrayUtil, kernel, declare, listen, has, miscUtil, TouchScroll, hasClass, put){
	...
	// hash for registering instances, referenced by List.registry functions
	var instances = {};
	...
	var List = declare(TouchScroll ? TouchScroll : null, {
		tabableHeader: false,
		// showHeader: Boolean
		...
		create: function(params, srcNodeRef){
			...
			// Also check for previously-registered instance;
			// throw if one exists, add it if it doesn't.
			id = domNode.id || this.id || generateId();
			if(instances[id]){
				throw new Error("dgrid instance already exists with id '" + id + "'");
			}
			this.id = domNode.id = id;
			instances[id] = this;
	...
	List.registry = {
		...
		findEnclosing: function(node){
			// summary:
			//		Returns the instance that the given node lies within.  If no such
			//		instance is found, a falsy value is returned.
