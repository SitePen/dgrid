...
//
//		dojoMobile.scrollable is a simple function object, which holds
...
//		widgets. dojoMobile._ScrollableMixin is such a class.
//
...
//		| function onLoad(){
//		| 	var scrollable = new dojoMobile.scrollable();
//		| 	scrollable.init({
...
var dojo = {isWebKit: true};
var dojoMobile = {};

var elem = document.createElement("div");
elem.style.webkitTransform = "translate3d(0px,1px,0px)";
document.documentElement.appendChild(elem);
var v = document.defaultView.getComputedStyle(elem, '')["-webkit-transform"];
dojoMobile.hasTranslate3d = v && v.indexOf("matrix") === 0;
document.documentElement.removeChild(elem);

dojoMobile.hasTouch = (typeof document.documentElement.ontouchstart != "undefined" &&
...
return function(){
	...
	dojo.create = function(tag, attrs, refNode){
		return refNode.appendChild(document.createElement(tag));
	};
	...
	this.init = function(/*Object?*/params){
			...
			for(var p in params){
				if (params.hasOwnProperty(p)) {
					this[p] = ((p == "domNode" || p == "containerNode") && typeof params[p] == "string") ?
						document.getElementById(params[p]) : params[p]; // mix-in params
				}
			}
		}
		...
		this._ch.push(dojo.connect(this.containerNode,
			dojoMobile.hasTouch ? "touchstart" : "onmousedown", this, "onTouchStart"));
		if(dojo.isWebKit){
...
/*		if(window.onorientationchange !== undefined){
			this._ch.push(dojo.connect(window, "onorientationchange", this, "resizeView"));
		}else{
			this._ch.push(dojo.connect(window, "onresize", this, "resizeView"));
		}
		this.resizeView();*/
		var _this = this;
		setTimeout(function(){
	...
	this.resizeView = function(e){
		...
		var id = setInterval(function() {
			...
			_this.domNode.style.height = (window.innerHeight||document.documentElement.clientHeight) - _this._appFooterHeight + "px";
	...
	this.onTouchStart = function(e){
		...
		if(!this._conn){
			this._conn = [];
			this._conn.push(dojo.connect(document, dojoMobile.hasTouch ? "touchmove" : "onmousemove", this, "onTouchMove"));
			this._conn.push(dojo.connect(document, dojoMobile.hasTouch ? "touchend" : "onmouseup", this, "onTouchEnd"));
		}
	...
	this.onTouchEnd = function(e){
		...
		var to = {}; // destination
		var dim = this._dim;
		var velocity = Math.sqrt(speed.x+speed.x + speed.y*speed.y);;
		
		if(this._v){
			to.y = pos.y + speed.y * velocity / 200;
		}
		if(this._h || this._f){
			to.x = pos.x + speed.x * velocity / 200;
		}
	...
	this.makeTranslateStr = function(to){
		...
		return dojoMobile.hasTranslate3d ?
	...
	this.getPos = function(){
		...
		if(dojo.isWebKit){
			var m = document.defaultView.getComputedStyle(this.containerNode, '')["-webkit-transform"];
	...
	this.hideScrollBar = function(){
		...
		if(this.fadeScrollBar && dojo.isWebKit){
			if(!dojoMobile._fadeRule){
				var node = dojo.create("style", null, document.getElementsByTagName("head")[0]);
				node.textContent =
					...
					"  50% { opacity: 0.6; }"+
					"  to { opacity: 0; }}";
				dojoMobile._fadeRule = node.sheet.cssRules[1];
			}
			fadeRule = dojoMobile._fadeRule;
		}
		if(!this.scrollBar){ return; }
		var f = function(bar){
			dojo.style(bar, {
...
//>>excludeEnd("webkitMobile");
	...
	this.createMask = function(){
		...
		if(this._scrollBarWrapperV){
			...
			ctx = document.getCSSCanvasContext("2d", "scrollBarMaskV", 5, h);
		...
		if(this._scrollBarWrapperH){
			...
			ctx = document.getCSSCanvasContext("2d", "scrollBarMaskH", w, 5);
			ctx.fillStyle = "rgba(0,0,0,0.5)";
...
//>>excludeStart("webkitMobile", kwArgs.webkitMobile);
		if(!dojoMobile.hasTouch && !this.noCover){
			if(!this._cover){
				this._cover = dojo.create("div", null, document.body);
				...
				this._ch.push(dojo.connect(this._cover,
					dojoMobile.hasTouch ? "touchstart" : "onmousedown", this, "onTouchEnd"));
			}else{
...
//>>excludeStart("webkitMobile", kwArgs.webkitMobile);
		if(!dojoMobile.hasTouch && this._cover){
