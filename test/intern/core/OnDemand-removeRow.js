define([
	...
	test.suite("removeRow", function(){
		...
		test.test("OnDemandList w/observers - remove all, clean up only", function(){
			...
			assert.isFalse(!!list._observers[0], "First observer should not exist.");
			assert.isTrue(!!list._observers[1], "Second observer should exist.");
			assert.isTrue(!!list._observers[2], "Third observer should exist.");
		});
