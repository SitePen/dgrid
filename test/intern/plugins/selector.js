define([
	...
	test.suite("selector column plugin", function () {
		test.test("programmatic row selection", function () {
			...
			var rowNode;
			var checkboxNode;
			var rowCount = testStore.data.length;
			var rowIndex = 0;
			var expectedSelection = {};
			var lastSelectedRow;

			window.grid = grid = new declare([OnDemandGrid, Selection])({
				store: testStore,
				columns: {
					select: selector({
						label: "Select"
			...
			for (rowIndex = 0; rowIndex < rowCount; rowIndex++) {
				...
				assert.deepEqual(grid.selection, expectedSelection, "grid.selection object should match expected values");
				...
				if (rowNode) {
					...
					assert.ok(checkboxNode.checked, "Checkbox for selected row " + rowIndex + " should be checked");
				}
			}
			...
			for (rowIndex = 0; rowIndex < rowCount; rowIndex++) {
				...
				if (rowNode) {
					...
					// if 'rowNode' exists
					checkboxNode = query(".field-select input", rowNode)[0];
					assert.isTrue(!checkboxNode.checked, "Checkbox for selected row " + rowIndex + " should NOT be checked");
				}
			}
		});
	});
