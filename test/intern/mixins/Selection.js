...
], function(test, assert, declare, arrayUtil, lang, domClass, Memory, Observable, Grid, OnDemandGrid, Selection, CellSelection, Pagination){
		...
		notificationTests[name + " + update + store + paging"] = function(){
			...
			function checkStyles(){
				...
				for(var id in grid._rowIdToObject){
					...
					var rowObject = grid._rowIdToObject[id];
					if(rowElement){
						var isHighlighted;
						if(name === "Selection"){
						...
						var shouldBeHighlighted = !!selection[rowObject.id];
