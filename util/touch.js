define(["dojo/on", "dojo/has", "dojo/query", "xstyle/css!../css/has-transforms3d.css"],
function(on, has, query){
	...
	has.add("transitionend", function(){
		// infers transitionend prefix based on CSS transitions has-feature
		var tpfx = has("css-transitions");
		if(!tpfx){ return false; }
		if(tpfx === true){ return true; }
		return { ms: "MS", O: "o", Moz: true, Webkit: "webkit" }[tpfx];
	});
	...
	has.add("css-transforms3d", function(){
		...
		// TODO: add CSS w/ media queries for tests
	...
	// define and populate module return object
